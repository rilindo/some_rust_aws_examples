...
use rusoto_sqs::{
    DeleteMessageRequest, GetQueueUrlRequest, GetQueueUrlResult, ReceiveMessageRequest,
    ReceiveMessageResult, Sqs, SqsClient,
};
...
async fn delete_message(
    ...
    let delete_message_req = DeleteMessageRequest {
        queue_url: queue_url
            ...
            .unwrap_or("default string")
            .to_string(),
        receipt_handle: message.messages.as_ref().unwrap()[0]
            .receipt_handle
            .as_ref()
            .unwrap()
            .to_string(),
...
fn main() {
    ...
    rt.block_on(delete_message(
        &client,
        &queue_url_resp,
        &received_message_resp,
    ));
}
