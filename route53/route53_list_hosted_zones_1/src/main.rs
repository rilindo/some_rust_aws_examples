...
async fn list_hosted_zones(client: &Route53Client, list_hosted_zones_req: ListHostedZonesRequest) -> ListHostedZonesResponse {

    let resp = client.list_hosted_zones(list_hosted_zones_req).await;
    return resp.unwrap();
...
fn main() {
    ...
    let list_hosted_zones_req = ListHostedZonesRequest {
        ..Default::default()
    };

    let mut rt = tokio::runtime::Runtime::new().unwrap();
    let mut resp = rt.block_on(list_hosted_zones(&client, list_hosted_zones_req));
    ...
    while resp.clone().is_truncated {
        ...
        resp = rt.block_on(list_hosted_zones(&client, list_hosted_zones_req));
        process_list_hosted_zones(resp.clone());
    }
}
