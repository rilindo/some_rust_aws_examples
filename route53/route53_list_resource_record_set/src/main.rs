...
async fn list_resource_record_sets(client: &Route53Client, list_resource_record_sets_request: ListResourceRecordSetsRequest) -> ListResourceRecordSetsResponse {

    let resp = client.list_resource_record_sets(list_resource_record_sets_request).await;
    return resp.unwrap();
...
fn process_list_resource_record_sets(resp: ListResourceRecordSetsResponse) {
    ...
    for r in resp.resource_record_sets {
        ...
        match r.alias_target {
            Some(alias_target) => println!("Record name: {}, DNS Target: {}", r.name, alias_target.dns_name),
            None => ()
        }
    }
...
fn main() {
    ...
    let mut resp = rt.block_on(list_resource_record_sets(&client, list_resource_record_sets_request));
    ...
    while resp.clone().is_truncated {
        ...
        resp = rt.block_on(list_resource_record_sets(&client, list_resource_record_sets_request));
        process_list_resource_record_sets(resp.clone());
    }
